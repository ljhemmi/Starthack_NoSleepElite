...
compare activity to database and classify
```

Sleep (Hours spent off of phone without interruption, sleep measurements using heartbeat)

```{r}
input: table measurements (table with bpm and date/time)
if (heartrate lower than 50 bpm) {start sleep session}
while (heartrate lower than 50 bpm)
{ sum minutes passed
if minutes larger than 480 classify enough sleep
```
...
```{r}
input photo of meal, database known foods and nutritional value
use bite.ai returns ranked list of most likely recognized foods 
compare list of recognized to database and create table with contents of meal
...
calculate calories of whole meal
```
...
```{r}
#sleep tracker
input: table with recorded sleep times
if average hours of sleep lower than 7 hours set enoughsleep to false
if enoughsleep is false or user input problemsleeping is true show caffeine log, screen-time track, recommend meditation guide
```

```{r}
#caffeine log
input: table with time and amount of consumed coffee, integer tired, integer bedtime, integer caffeine_metabolism = 6
if tired bigger than bedtime set reminder at (bedtime-caffeine_metabolism to stop caffeine consumption
individualization
if (tired-latest_consumption) unequal to caffeine_metabolims set caffeine_metabolism to (tired-latest_consumption)
```
...
#hectic days
input: int stepcount
if stepcount bigger than 20'000 alert "You have reached 20'000 steps, your day seems to be rather hectic. We invite you to take a quick break and breathe in deeply."
```
...
```{r}
input: jpg photo
use bite.ai returns ranked list with most likely recognized foods
if classified foods cluster throw PersonEatsOnlyXYZ Exception
catch PersonEatsOnlyXYZ Exception System.out.print("We recommend you the following recipes to ensure all nutritional needs are taken care of")
```
